...
from .models import Team, Organisation

"""
    Here are three forms we can use to add everything a user will need,
    organisations, teams and the user.
    NOTE: At some point we'll be getting the user from LDAP (or somewhere
    else), at which point we'll generate them a matching db record and then
    ask them to assign some teams to themselves.

    As a bonus we now have pages where organisations and teams can be added
    /organisations/new
    /teams/new
    /users/new
"""
...
#   ORGANISATIONS
#
#   The simple new Organisation form
...
#   TEAMS
#
#   The slightly more complicated TeamForm, not to be confused with
#   TeamFortress, this has less hats.
